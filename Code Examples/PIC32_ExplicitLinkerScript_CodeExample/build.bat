...
set TOOLS_DIR="C:\Program Files\Microchip\MPLAB C32\bin\"
...
REM "C:\Program Files\Microchip\MPLAB C32\bin\pic32-gcc.exe" -mprocessor=32MX360F512L -x c -c "src\main.c" -o"main.o" -MMD -MF"main.d" -D__DEBUG -g
REM "C:\Program Files\Microchip\MPLAB C32\bin\pic32-gcc.exe" -mprocessor=32MX360F512L "main.o" -o"explicit_linker_script.elf" -Wl,-L"ldscript",--script="ldscript\elf32pic32mx.ld",--defsym=__MPLAB_BUILD=1,--defsym=__MPLAB_DEBUG=1,-Map="explicit_linker_script.map"

REM Turn echo on so that user can see actual buld command we are executing
@echo on
%TOOLS_DIR%pic32-gcc %SOURCE% -mprocessor=%PROCESSOR% -c %INCLUDE_PATHS% %GCC_OPTIONS%
%TOOLS_DIR%pic32-gcc %OBJECTFILES%  -mprocessor=%PROCESSOR% -o %ELF_FILE_NAME% %LINKER_PATHS% %LD_OPTIONS%
